...
const sessionUpdate = async (req, model) => {
  ...
  if (token && token.length) {
    const expiredtime = getExpiredtime();
    const ses = await model
      .findOneAndUpdate({ token }, { expiredtime })
      .populate("user_id");
    if (ses) {
      return ses.user_id;
    } else {
...
const getExpiredtime = () => {
  const expiredtime = new Date(
    new Date().valueOf() + parseInt(config.USER_SESSION_TIME)
  );
  return expiredtime;
};
...
const updatePortfolioByTransaction = async (
  portfolio_id,
  update_data,
  model
) => {
  ...
  let result = await model.updateOne(
    { _id: portfolio_id },
    { $set: { transaction: transaction_data } }
  );
  return result;
};
